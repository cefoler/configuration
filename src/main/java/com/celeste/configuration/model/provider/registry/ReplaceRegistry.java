...
public final class ReplaceRegistry implements Serializable, Cloneable {
  ...
  @Nullable
  public ReplaceValue registerIfAbsent(@NotNull final String key, @Nullable final ReplaceValue value) {
  ...
  public Set<Entry<String, ReplaceValue>> getEntrySet(@NotNull final ReplaceType type) {
    return map.entrySet().stream()
        .filter(entry -> entry.getValue().getType().equals(type) || entry.getValue().getType().equals(ReplaceType.ALL))
  ...
  public Set<ReplaceValue> getAll(@NotNull final ReplaceType type) {
    return map.values().stream()
        .filter(replace -> replace.getType().equals(type) || replace.getType().equals(ReplaceType.ALL))
        .collect(Collectors.toSet());
  }
